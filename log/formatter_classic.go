...
func prefixFieldClashes(data logrus.Fields, ignoreFields []string, hasCaller bool) {
...
type ClassicFormatter struct {
	...
	IgnoreFields []string // 忽略的 fields
}
...
func stripPathAuto(path string) string {
	...
	length := len(parts)
	if length > 2 {
		return strings.Join(parts[length-2:], "/")
	}
	return path
}
...
func (f *ClassicFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	prefixFieldClashes(entry.Data, f.IgnoreFields, entry.HasCaller())
...
var _ = func() interface{} {
	registerFormatter("classic", reflect.TypeOf(ClassicFormatter{}))
	return nil
}()
